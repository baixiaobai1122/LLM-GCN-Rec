...
class BookMetadataFetcher:
    def __init__(self, cache_file='../data/amazon-book/book_metadata.json'):
    ...
    def fetch_all_books(self, isbn_list, batch_size=100, delay=0.5):
        ...
        if failed:
            ...
            with open('../data/amazon-book/failed_isbns.txt', 'w') as f:
                f.write('\n'.join(failed))

        return self.metadata

def load_isbn_list(item_list_file='../data/amazon-book/item_list.txt'):
...
def main():
    ...
    parser.add_argument('--sample', type=int, default=100,
                        help='Number of books to fetch (default: 100, use -1 for all)')
    args = parser.parse_args()

    # 1. 加载ISBN列表
    isbns = load_isbn_list()
    ...
    # 3. 获取元数据
    if args.sample == -1:
        print(f"📌 Fetching all {len(isbns)} books (~13 hours)")
    else:
        isbns = isbns[:args.sample]
        print(f"📌 Fetching first {len(isbns)} books")

    # 开始获取
    metadata = fetcher.fetch_all_books(isbns, batch_size=100, delay=0.5)
